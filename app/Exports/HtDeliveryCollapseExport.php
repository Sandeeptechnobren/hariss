    ...
    public function collection()
{
    ...
    $headers = HTDeliveryHeader::with([
         ...
         $q->whereDate('delivery_date', '>=', $this->fromDate)
         ->whereDate('delivery_date', '<=', $this->toDate);
        })
    ...
    foreach ($headers as $header) {
        ...
        $rows[] = [
            ...
            $deliveryDate,
            optional($header->poorder)->order_code ?? '',
        ...
        $rows[] = [
            '',
            'Item','UOM Name','Item Price','Quantity',
            'Net Detail','Excise Detail','Detail VAT','Detail Total','',''
        ];
        $rowIndex++;
        $start = $rowIndex - 1;
        ...
        foreach ($details as $d) {
            $rows[] = [
                '',
                trim((optional($d->item)->erp_code ?? '') . ' - ' . (optional($d->item)->name ?? '')),
                optional($d->uoms)->name ?? '',
                $d->item_price ? number_format($d->item_price,2) : '',
                $d->quantity ?? '',
