...
class RegionController extends Controller
{
    ...
    {
        $filters = $request->only(['region_name', 'region_code', 'status', 'country_id']);
        $perPage = $request->get('limit', 10);

        $regions = $this->regionService->getAll($perPage, $filters);
    ...
    public function regionDropdown(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'code'    => 200,
            'message' => 'Regions fetched successfully',
            'data'    => $regions->items(), // sirf select kiye gaye columns
            'pagination' => [
                'page'         => $regions->currentPage(),
                'limit'        => $regions->perPage(),
                'totalPages'   => $regions->lastPage(),
     ...
     *         @OA\JsonContent(
     *             type="object",
     *             required={"region_name","country_id"},
...
{
    try {
        $data = $request->all(); // or $request->validated() if using FormRequest
 ...
 *     description="Search by region_name, region_code, status, country, created_user, updated_user, etc. Returns paginated results.",
 *     security={{"bearerAuth":{}}},
 *     @OA\Parameter(
 *         name="query",
 *         in="query",
 ...
 *                         @OA\Property(property="country_code", type="string", example="IN")
 *                     ),
 *                     @OA\Property(property="status", type="integer", example=1),
