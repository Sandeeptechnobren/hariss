<?php
// app/Http/Controllers/Api/UraController.php

namespace App\Http\Controllers\V1\EfrisAPI;

use App\Http\Controllers\Controller;
use App\Services\V1\EfrisAPI\EfrisStockSyncService;
...
class EfrisStockSyncTestController extends Controller
{
    // protected $service;

    // public function __construct(EfrisStockSyncService $service)
    // {
    //     $this->service = $service;
    // }

    public function syncItems_test(Request $request)
    {
        $itemId = $request->input('item_id', 'all');
        $result = $this->syncItems1($itemId);

        return response()->json($result);
    }

    private function syncItems1($itemId)
    {
        ...
        $warehouses = Warehouse::where('is_efris', 1)->where('status', 1)->get();
    ...
    private function fetchData($warehouse = null)
    {
        ...
        return array("data" => array("content" => "", "signature" => "", "dataDescription" => array("codeType" => "0", "encryptCode" => "1", "zipCode" => "0")), "globalInfo" => array("appId" => "AP04", "version" => "1.1.20191201", "dataExchangeId" => "9230489223014123", "interfaceCode" => "T101", "requestTime" => date("Y-m-d H:i:s"), "requestCode" => "TP", "responseCode" => "TA", "userName" => "admin", "deviceMAC" => "FFFFFFFFFFFF", "deviceNo" => "", "tin" => "$get_tins", "brn" => "", "taxpayerID" => "1", "longitude" => $warehouse->longitude, "latitude" => $warehouse->latitude, "extendField" => array("responseDateFormat" => "dd/MM/yyyy", "responseTimeFormat" => "dd/MM/yyyy HH:mm:ss")), "returnStateInfo" => array("returnCode" => "", "returnMessage" => ""));
    }
