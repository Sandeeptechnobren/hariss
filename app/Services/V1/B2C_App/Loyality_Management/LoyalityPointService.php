...
namespace App\Services\V1\B2C_App\Loyality_Management;
...
class LoyalityPointService
{
    ...
    {
        $summary = CustomerLoyalityPoint::where('customer_id', $customerId)
            ->select(
                'customer_id',
                'total_earning',
                'total_spend',
            ...
            ->first();
        // dd($summary);
        $invoicePoints = CustomerLoyalityActivity::where('customer_id', $customerId)
            ->select(
                'invoice_id',
                'activity_date',
                'incooming_point',
    ...
    //             $quantities[$detail->item_id] =
    //                 ($quantities[$detail->item_id] ?? 0) + $qty;

    //             if ($item->volumes > 0 && $qty >= $item->volumes) {
    //                 $perVolumeReward = $item->rewards / $item->volumes;
    ...
    //             if ($bonus && $bonus->volume > 0 && $totalQty >= $bonus->volume) {
    ...
    //         $lastClosing = $loyalty->total_closing ?? 0;
