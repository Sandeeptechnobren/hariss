...
{
    $query = CompanyCustomer::select('id', 'customer_code', 'business_name', 'merchendiser_ids')
    ...
    if (!empty($merchandiserIds)) {
        $query->where(function ($q) use ($merchandiserIds) {
            foreach ($merchandiserIds as $id) {
                $q->orWhereRaw("CONCAT(',', merchendiser_ids, ',') LIKE ?", ["%,$id,%"]);
            }
        });
    }
        ...
        $query->where(function ($q) use ($like, $searchTerm) {
            ...
            foreach (['createdUser', 'updatedUser', 'deletedUser'] as $relation) {
                $q->orWhereHas($relation, fn($sub) =>
                    $sub->whereRaw('LOWER(firstname) LIKE ?', [$like])
            ...
            $matchingCustomerIds = \App\Models\CompanyCustomer::where(function ($sub) use ($like) {
                $sub->whereRaw('LOWER(customer_code) LIKE ?', [$like])
...
public function importFromCsv($file)
{
 ...
 public function getFilteredShelves($filters)
    {
        $query = Shelve::query();

        // Apply only date filters
        if (!empty($filters['start_date'])) {
            $query->where('valid_from', '>=', $filters['start_date']);
        }

        if (!empty($filters['end_date'])) {
            $query->where('valid_to', '<=', $filters['end_date']);
        }

        $shelves = $query->get();

       return $shelves->flatMap(function ($shelve) {
    // These are already arrays due to model casting
    $merchandiserIds = $shelve->merchendiser_ids ?? [];
    $customerIds     = $shelve->customer_ids ?? [];

    // Fetch names from related tables
    $merchandisers = Salesman::whereIn('id', $merchandiserIds)->pluck('name', 'id');
    $customers     = CompanyCustomer::whereIn('id', $customerIds)->pluck('business_name', 'id');

    // Build the cartesian product
    return collect($merchandiserIds)->flatMap(function ($merchId) use ($customerIds, $merchandisers, $customers, $shelve) {
        return collect($customerIds)->map(function ($customerId) use ($merchId, $merchandisers, $customers, $shelve) {
            return [
                'shelf_name'        => $shelve->shelf_name,
                'code'              => $shelve->code,
                'height'            => $shelve->height,
                'width'             => $shelve->width,
                'depth'             => $shelve->depth,
                'valid_from'        => $shelve->valid_from,
                'valid_to'          => $shelve->valid_to,
                'customer_name'     => $customers[$customerId] ?? 'N/A',
                'merchandiser_name' => $merchandisers[$merchId] ?? 'N/A',               
            ];
        });
    });
})->values();
