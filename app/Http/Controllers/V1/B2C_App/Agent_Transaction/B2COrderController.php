...
namespace App\Http\Controllers\V1\B2C_App\Agent_Transaction;
...
use App\Http\Requests\V1\B2C_App\Agent_Transaction\B2CStoreOrderRequest;
use App\Http\Resources\V1\B2C_App\Agent_Transaction\B2COrderHeaderResource;
use App\Services\V1\B2C_App\Agent_Transaction\B2COrderService;

class B2COrderController extends Controller
{
    use ApiResponse;

    protected B2COrderService $service;
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $orders = $this->service->getAll($query);
     ...
     * )
     */
    public function store(B2CStoreOrderRequest $request): JsonResponse
    {
        // dd($request->all());
        try {
            ...
            return response()->json([
                'status' => 'success',
                'code' => 201,
                'data' => new B2COrderHeaderResource($order)
            ], 201);
        } catch (Exception $e) {
            return response()->json([
                'status' => 'error',
    ...
    public function exportOrderHeader(Request $request)
    {
        ...
        $filename = 'Agent_order_' . now()->format('Ymd_His') . '.' . $extension;
        $path = 'orderexports/' . $filename;

        // 🔥 GET FULL FILTER OBJECT
    ...
    public function exportOrders($uuid)
    {
        $orderHerader = OrderHeader::select('order_code')
            ->where('uuid', $uuid)
            ->first();

        $code = $orderHerader?->order_code ?? 'UNKNOWN';

        $code = preg_replace('/[^A-Za-z0-9\-]/', '', $code);

        $filename = 'Agent_order_' . $code . '.pdf';
    ...
    public function exportCollapseOrders(Request $request)
    {
        $format = strtolower($request->input('format', 'xlsx'));
        $extension = $format === 'csv' ? 'csv' : 'xlsx';
        $filename = 'Agent_order_detail_' . now()->format('Ymd_His') . '.' . $extension;
        $path = 'orderexports/' . $filename;
        ...
        // 🔥 SAME LOCATION HELPER (:contentReference[oaicite:0]{index=0})
        $warehouseIds = CommonLocationFilter::resolveWarehouseIds($filters);
