...
class CompanyService
{
    ...
    public function getAll($perPage = 10, $filters = [])
        {
            try {
                $query = Company::with('country', 'getregion', 'getarea')->orderByDesc('created_at');
    ...
    {
        try {
            $query = Company::with(['country:id,country_name,country_code', 'getregion:id,region_name,region_code']);
...
public function delete(int $id): ?Company
    ...
    public function bulkUpload($file){
        ...
        try {
            ...
            unset($sheetData[1]);
            $expectedHeaders = [
                'company_name', 'email', 'tin_number', 'vat', 'selling_currency',
                'purchase_currency', 'toll_free_no', 'website', 'district', 'town',
                'street', 'landmark', 'region_id', 'sub_region_id', 'primary_contact',
                'country_id', 'service_type', 'company_type', 'status'
            ];
