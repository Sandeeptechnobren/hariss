...
namespace App\Http\Resources\V1\B2C_App\Agent_Transaction;

use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

class B2COrderDetailResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'item_uoms' => $this->itemUom ? [
                'id'       =>  $this->itemUom->id,
                'name'     => optional($this->itemUom->uom)->name, // 🔥 FROM UOM TABLE
                'uom_id'   => $this->itemUom->uom_id,
                'uom_type' => $this->itemUom->uom_type,
                'upc'      => $this->itemUom->upc,
            ] : null,

            'children' => B2COrderDetailResource::collection($this->whenLoaded('children')),
